...
#else
static __be32 find_leastused_ip(const u16 zone, const struct nf_nat_range *range, const __be32 src, const __be32 dst)
#endif
{
  /* Host order */
  u32 minip = 0, maxip = 0, j, dist;
...
#else
  j = jhash_1word((u32)src, range->flags & IP_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone);
#endif
  dist  = maxip - minip + 1;
  ...
  if (xt_hooknum(par) == NF_INET_PRE_ROUTING) {
    ...
    if (protonum == IPPROTO_UDP) {
      ...
      if (src_mapping != NULL && check_mapping(src_mapping, net, zone)) {
        ...
        want_port = find_appropriate_port(net, zone, original_port, newrange.min_ip, range);
#endif
#if LINUX_VERSION_CODE > KERNEL_VERSION(3, 10, 0)
        newrange.flags = NF_NAT_RANGE_MAP_IPS | NF_NAT_RANGE_PROTO_SPECIFIED;
        newrange.min_proto.udp.port = cpu_to_be16(want_port);
