...
int __attribute__((weak)) check_lock_status(time_t *dt)
{
	...
	if ( cur_login_ip_type )
	{
		...
		*dt = login_timestamp_tmp_wan - nvram_get_int(&HTTPD_LAST_LOGIN_TS_W);
		if ( nvram_get_int(&HTTPD_LAST_LOGIN_TS_W) && *dt > max_lock_time )
		{
...
//			HTTPD_DBG("timeout");
		...
		if ( nvram_get_int(&HTTPD_LOGIN_FAIL_WAN) <= 4 )
			return 0;
		lock_flag |= 2u;
		if ( !(nvram_get_int(&HTTPD_LOGIN_FAIL_WAN) % 5) )
		{
			temp_ip_addr.s_addr = login_ip_tmp;
			logmessage_normal("httpd login lock",  "Detect abnormal logins at %d times. The newest one was from %s in login lock.",
				nvram_get_int(&HTTPD_LOGIN_FAIL_WAN), inet_ntoa(temp_ip_addr));
			return 7;
		}
	}
	else
	{
		...
		*dt = login_timestamp_tmp - nvram_get_int(&HTTPD_LAST_LOGIN_TS);
		if ( nvram_get_int(&HTTPD_LAST_LOGIN_TS) && *dt > max_lock_time )
		{
			nvram_set_int(&HTTPD_LOGIN_FAIL_LAN, 0);
			nvram_set_int(&HTTPD_LAST_LOGIN_TS, 0);
			lock_flag &= 0xFFFFFFFE;
			login_error_status = 0;
			nvram_set_int(&CAPTCHA_FAIL_NUM, 0);
//			HTTPD_DBG("timeout");
		}
		if ( nvram_get_int(&HTTPD_LOGIN_FAIL_LAN) <= 4 )
			return 0;
		lock_flag |= 1u;
		if ( !(nvram_get_int(&HTTPD_LOGIN_FAIL_LAN) % 5) )
		{
			temp_ip_addr.s_addr = login_ip_tmp;
			logmessage_normal("httpd login lock", "Detect abnormal logins at %d times. The newest one was from %s in login lock.",
				nvram_get_int(&HTTPD_LOGIN_FAIL_LAN),inet_ntoa(temp_ip_addr));
			return 7;
		}
	}
	return 7;
