...
int lib1x_resolve()
{
	...
	memset(ourip, '\0', IP_ADDRSIZE);
	strncpy(ourip, inet_ntoa(tmpip), IP_ADDRSIZE-1);
...
*/
	...
	memset(dev_supp, '\0', LIB1X_MAXDEVLEN);
	strncpy(dev_supp, theArgs[1], LIB1X_MAXDEVLEN-1);
...
#ifdef CONFIG_YUEME
	else if(!strcmp(theArgs[2], "any")){
		_AUTH_DBGMSG("Radius interface : any");
		dev_svr[0] = '\0';
	}
	else{
#endif
	_AUTH_DBGMSG("Radius interface : %s", theArgs[2]);
	memset(dev_svr, '\0', LIB1X_MAXDEVLEN);
	strncpy(dev_svr, theArgs[2], LIB1X_MAXDEVLEN-1);
#ifdef CONFIG_YUEME
	}
#endif
	...
	memset(dev_supp, '\0', LIB1X_MAXDEVLEN);
	strncpy(dev_supp, "wlan0", sizeof("wlan0"));
