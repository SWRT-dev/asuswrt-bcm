...

{
  ...
  /* Set dev name */
  memset(&wrq.ifr_name, '\0', IFNAMSIZ);
  strncpy(wrq.ifr_name, ifname, (IFNAMSIZ-1));
...

	{
	  ...
	  if(!strcasecmp(args[i], "on"))
...

		{
		  ...
		  if(wrq.u.data.length > (IW_ENCODING_TOKEN_MAX-1))

		    wrq.u.data.length = IW_ENCODING_TOKEN_MAX-1;
...

		{
		  ...
		  memset(buff, 0, strlen(args[i])+1);
		  strncpy(buff, args[i], strlen(args[i]));
...

	{
	  ...
	  if((!strcasecmp(args[i], "off")) ||
...

		{
		  ...
		  strncpy(essid, args[i], IW_ESSID_MAX_SIZE);	/* Size checked, all clear */
