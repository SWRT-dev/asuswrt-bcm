	...
	case IW_PRIV_TYPE_CHAR:
	  if(i < count)
		...
		wrq.u.data.length = priv[k].set_args & IW_PRIV_SIZE_MASK;
	      ...
	      /* Fetch string */
	      if (wrq.u.data.length > (sizeof(buffer) - 1)) {
			printf("set_private IW_PRIV_TYPE_CHAR Error: data.length (%d) exceeds the buffer size\n", wrq.u.data.length);
			return (-1);
	      }
	    ...
	    char *	ifname)		/* Dev name */
{
  ...
  if(ChangeRoamState)
      ...
      if(k == number)
	{
      ...
      memcpy(wrq.u.name, &buffer, (IFNAMSIZ-1));
