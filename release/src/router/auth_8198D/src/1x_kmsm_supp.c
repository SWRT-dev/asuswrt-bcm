...
extern int DecWPA2KeyData(Supp_Global *pGlobal, u_char *key, int keylen, u_char *kek, int keklen, u_char *kout, int kout_len);
...
void lib1x_suppsm_capture_control( Supp_Global * pGlobal, lib1x_nal_intfdesc_tag * nal, lib1x_packet_tag * spkt )
{
	...
	case DOT11_EVENT_ASSOCIATION_IND:
		...
		if (pGlobal->auth->UsePassphrase) {
			lib1x_control_STA_QUERY_SSID(pGlobal, ssid, sizeof(ssid));
			if (strcmp(pGlobal->auth->RSNVariable.ssid, ssid)) {
				memset(pGlobal->auth->RSNVariable.ssid, '\0', sizeof(pGlobal->auth->RSNVariable.ssid));
				strncpy(pGlobal->auth->RSNVariable.ssid, ssid, sizeof(pGlobal->auth->RSNVariable.ssid)-1);
...
if ( pGlobal->auth->RSNVariable.WPA2Enabled && Message_DescType(pGlobal->EapolKeyMsgRecvd) == desc_type_WPA2)
{
			...
			//DecGTK(pGlobal->EAPOLMsgRecvd, u_char *kek, int keklen, int keylen,u_char *kout, int kout_len)
...
//			if(!DecWPA2KeyData(pKeyData, keyDataLength, pGlobal->supp_kmsm->PTK + PTK_LEN_EAPOLMIC, PTK_LEN_EAPOLENC
	...
	else if( Message_KeyType(pGlobal->EapolKeyMsgRecvd) == type_Group)
	{
		...
		}else if(!CheckMIC(pGlobal->EAPOLMsgRecvd, pGlobal->supp_kmsm->PTK, PTK_LEN_EAPOLMIC))
		{
			...
			pGlobal->supp_kmsm->GTK[Message_KeyIndex(pGlobal->EapolKeyMsgRecvd)], GTK_LEN))
		{
		...
		}else
		{
			...
			if ( pGlobal->auth->RSNVariable.WPA2Enabled && 
				...
				if (Message_KeyDataLength(pGlobal->EapolKeyMsgRecvd) > sizeof(decrypted_data)) {
					printf("Message 2-1: keyID length (%d) exceeds! %d\n", Message_KeyDataLength(pGlobal->EapolKeyMsgRecvd), __LINE__);
					return;
				}
