...
libnet_open_link_interface(char *device, char *ebuf)
{
    ...
    if (l == NULL)
    {
        snprintf(ebuf, LIBNET_ERRBUF_SIZE, "malloc: %s", strerror(errno));
        return (NULL);
    }
    memset(l, 0, sizeof (*l));
    ...
    {
        snprintf(ebuf, LIBNET_ERRBUF_SIZE, "SIOCGIFHWADDR: %s", strerror(errno));
        goto bad;
    }
    ...
    switch (ifr.ifr_hwaddr.sa_family)
    {
        ...
        case ARPHRD_PPP:
            ...
            break;
        default:
            snprintf(ebuf, LIBNET_ERRBUF_SIZE, "unknown physical layer type 0x%x",
...
libnet_get_hwaddr(struct libnet_link_int *l, const char *device, char *ebuf)
{
    ...
    if (fd < 0)
    {
        snprintf(ebuf, LIBNET_ERRBUF_SIZE, "get_hwaddr: %s", strerror(errno));
        return (NULL);
    }
