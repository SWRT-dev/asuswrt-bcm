...
void EncGTK(Global_Params * global, u_char *kek, int keklen, u_char *key,
			int keylen, u_char *out, int out_size, u_short *outlen)
{
...
//	}else if(Message_KeyDescVer(global->EapolKeyMsgRecvd) == key_desc_ver2)
	}else if(Message_KeyDescVer(global->EapolKeyMsgSend) == key_desc_ver2)
	{
			...
			AES_WRAP(key, keylen, default_key_iv, 8, kek, keklen, out, out_size, outlen);
...
//			AES_UnWRAP(EapolKeyMsgRecvd.Octet + KeyDataPos, keylen + 8, kek, keklen, tmp2, sizeof(tmp2));
