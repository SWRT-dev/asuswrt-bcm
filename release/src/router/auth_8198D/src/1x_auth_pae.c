...
#endif
{
	...
	//----endif
	{
		...
		strncpy( dev_txrx->device_svr, dev_svr, IFNAMSIZ+10-1);
...
#else
		strncpy( dev_txrx->device_svr, dev_supp, IFNAMSIZ+10-1);
#endif
...
#ifdef CONFIG_YUEME
		if(lib1x_nal_connect( dev_txrx->network_svr,  & dev_txrx->radsvraddr, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_AUTH, dev_svr) < 0)
#else
		...
		#endif
#ifdef CONFIG_YUEME
		if(dev_txrx->network_svr->udpsock_connected){
			lib1x_nal_getIpaddress(dev_txrx->network_svr, &dev_txrx->ourip_inaddr.s_addr);
		}
#else
...
#ifdef CONFIG_YUEME
			if(lib1x_nal_connect( dev_txrx->network_svr2,  & dev_txrx->radsvraddr2, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_AUTH, dev_svr) < 0)
#else
...
#ifdef CONFIG_YUEME
			if(lib1x_nal_connect( dev_txrx->network_svr,  & dev_txrx->acctsvraddr, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_ACCT, dev_svr) < 0)
#else
	...
	strncpy( dev_txrx->device_supp, dev_supp, IFNAMSIZ+10-1);
	...
	strncpy( dev_txrx->device_wlan0, dev_txrx->device_supp, IFNAMSIZ+10-1);			
...
int lib1x_init_auth(Dot1x_Authenticator * auth)
{
	...
	if(auth->AccountingEnabled)
	{
		...
		if (ulUTF8Len <= sizeof(auth->authGlobal->global->theAuthenticator->rinfo->username))
...
Global_Params *  lib1x_init_authenticator(Dot1x_Authenticator *auth, TxRx_Params * dev_txrx)
{
	...
	memset(auth_pae->rinfo->nas_identifier, '\0', MAX_NAS_ID_LEN);
	strncpy( auth_pae->rinfo->nas_identifier, lib1x_nas_id, MAX_NAS_ID_LEN-1); //sc_yang
	memset(auth_pae->rinfo->connectinfo, '\0', sizeof(auth_pae->rinfo->connectinfo));
	strncpy( auth_pae->rinfo->connectinfo, "CONNECT 11Mbps 802.11b", sizeof(auth_pae->rinfo->connectinfo)-1);
	auth_pae->rinfo->rad_stateavailable = FALSE;
	auth_pae->rinfo->rad_statelength = 0;

    auth_pae->enableUserId = auth->enableUserId;
    auth_pae->enableCalledId = auth->enableCalledId;
...
#else
	if (sizeof(global->auth->RSNVariable.PassPhraseKey) <= sizeof(global->PSK))
...
//				memset(&global->akm_sm->CurrentReplayCounter, '\0', sizeof(LARGE_INTEGER));
				...
				{	
					...
					strncpy(ifr.ifr_name, dev_supp, (IFNAMSIZ-1));
		...
		//ToDo: Process EAP Message
                {
                        ...
                        if (tmplen <= (LIB1X_MAXEAPLEN - auth_pae->rinfo->eap_messlen_frmserver))
...
void lib1x_process_radiuschallenge( Global_Params * global, struct lib1x_packet * spkt)
{
	...
	while (!done )
	{
		...
		if ( rattr->type == LIB1X_RAD_STATE )
		{
			...
			if ((rattr->length - 2) <= LIB1X_MAXEAPLEN)
