...
void AES_WRAP(u08b * plain, int plain_len, 
			  ...
			  u08b *cipher, int cipher_size, u16b *cipher_len)

{
	int		i, j, k, nblock = plain_len/BLOCKSIZE8;
#ifdef RTL_WPA2	
	if (nblock > 32) {
		printf("AES_WRAP Error: plain len (%d) exceeds!\n", plain_len);
		return;
	}
	...
	if (cipher_size < BLOCKSIZE8) {
		printf("AES_WRAP Error: cipher size (%d) is too small!\n", cipher_size);
		return;
	}

	//if (cipher_size < plain_len) {
	//	printf("AES_WRAP Error: plain len (%d) exceeds cipher size!\n", plain_len);
	//	return;
	//}

	if (kek_len > BLK_SIZE) {
		printf("AES_WRAP Error: kek_len (%d) exceeds BLK_SIZE!\n", kek_len);
		return;
	}
			    ...
			    u08b * plain, int plain_size)
{
	...
	if (plain_size < BLOCKSIZE8) {
		printf("AES_UnWRAP Error: plain size (%d) is too small!\n", plain_size);
		return;
	}

	if (plain_size < cipher_len) {
		printf("AES_UnWRAP Error: cipher len (%d) exceeds plain size!\n", cipher_len);
		return;
	}

	if (kek_len > BLK_SIZE) {
		printf("AES_UnWRAP Error: kek_len (%d) exceeds BLK_SIZE!\n", kek_len);
		return;
	}
	memcpy(&p.key.b , kek, kek_len); 
	AES_SetKey(p.key.x, BLK_SIZE*8);     // run the key schedule
