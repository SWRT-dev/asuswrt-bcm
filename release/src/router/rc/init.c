...
#if defined(RTAX3000N) || defined(RTAX55) || defined(RTAX1800)
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_RTAX55:
		...
		if (nvram_match("HwId", "B") || nvram_get_int("no_ed_thresh_unset") == 0) {
		...
		if (ATE_BRCM_FACTORY_MODE())
		...
		else if ((!nvram_get_int("x_Setting") || nvram_get_int("ed_thresh_force") || (!aimesh_re_node() && is_CN_sku() && nvram_match("location_code", "XX")) || (aimesh_re_node() && !strlen(nvram_safe_get("cfg_group")))) &&
			...
			nvram_set("no_dy_ed_thresh_ctrl", "-1");
			nvram_set("sb/0/ed_thresh2g", "-40");
			nvram_set("sb/1/ed_thresh5g", "-40");
		} else {
			nvram_unset("no_dy_ed_thresh_ctrl");
		}

		nvram_set_int("ed_thresh_reload", ((no_dy_ed_thresh_ctrl_old != nvram_get_int("no_dy_ed_thresh_ctrl")) || (ed_thresh5g_old != nvram_get_int("sb/1/ed_thresh5g"))));
