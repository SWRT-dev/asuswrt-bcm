...
static int add_credential_attr(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *out, int include_cred)
{
	...
	if (pCtx->config_state == CONFIG_STATE_UNCONFIGURED && 
			!(pCtx->is_ap && pCtx->invoke_security_gen==0)) {
...
#if defined(TRIBAND_WPS)		/*for wlan0-va0/wlan2 interface*/
static int add_credential_attr3(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *out, int include_cred)
{
    unsigned char byteVal, *pMsg;
    unsigned short shortVal;
    int size, idx=0;
    ...
    if (pCtx->config_state == CONFIG_STATE_UNCONFIGURED) {
        ...
        // ssid  setting
        if (pCtx->use_ie == 2) 			
            strcat(pCtx->SSID3, tmp1+(WSC_MAX_SSID_LEN-4));	
        else {
            if (!pCtx->disable_auto_gen_ssid) {
                memset(pCtx->SSID3, '\0', WSC_MAX_SSID_LEN+1);				
                if(strlen(pCtx->ssid_prefix)>0) {
                    char tmpstr[24];
                    convert_bin_to_str(pCtx->our_addr3, ETHER_ADDRLEN, tmpstr);	

                    strcpy(pCtx->SSID3, pCtx->ssid_prefix);						
                    strncat(pCtx->SSID3, tmpstr, 12);
                } else {
                    strcpy(pCtx->SSID3, "WPS");
                    strncat(pCtx->SSID3, tmp1, 6);
                    strncat(pCtx->SSID3, tmp1+(WSC_MAX_SSID_LEN-4), 4);						
                }
            }
        }
        DEBUG_PRINT("auto-generated SSID3: [%s]\n", pCtx->SSID3);

        // network key setting
        convert_bin_to_str(&tmpbuf[WSC_MAX_SSID_LEN/2], BYTE_LEN_256B, tmp1);		
        if (!pCtx->manual_key_type) {
            if(pCtx->random_psk_len>= 8 && pCtx->random_psk_len<=64) {
                memcpy(pCtx->network_key3, (unsigned char *)tmp1, pCtx->random_psk_len);
                pCtx->network_key3[pCtx->random_psk_len] = '\0';
                pCtx->network_key_len3 = pCtx->random_psk_len;
            } else {
                memcpy(pCtx->network_key3, (unsigned char *)tmp1, 22);
                pCtx->network_key3[22] = '\0';
                pCtx->network_key_len3 = 22;			
                //DEBUG_PRINT("PSK length no assigned use default\n");
            }			
            //DEBUG_PRINT("auto-generated netwok-key: %s\n", pCtx->network_key2);			
        } else {			
            ...
            // avoid error conf setting			
            if (strlen(pCtx->manual_key)>=MIN_NETWORK_KEY_LEN && strlen(pCtx->manual_key)<=MAX_NETWORK_KEY_LEN) {
                strcpy((char*)pCtx->network_key3, pCtx->manual_key);
                pCtx->network_key_len3 = strlen((char*)pCtx->network_key3);
                _DEBUG_PRINT("use manual netwok-key: %s\n", pCtx->network_key3);

            } else if(pCtx->manauall_random_psk_len) {
                memcpy(pCtx->network_key3, (unsigned char *)tmp1, pCtx->manauall_random_psk_len);
                pCtx->network_key3[pCtx->manauall_random_psk_len] = '\0';
                pCtx->network_key_len3 = pCtx->manauall_random_psk_len;
                _DEBUG_PRINT("use random gen netwok-key: %s , len = %d\n", 
                pCtx->network_key3 , pCtx->manauall_random_psk_len);
            } else if(strlen(pCtx->manual_key)<MIN_NETWORK_KEY_LEN ) {
                //at conf file "manual_key_type"!=0 but no assigned a valid PSK 
                strcpy((char*)pCtx->network_key3, "1234567890");
                pCtx->network_key_len3 = strlen("1234567890");
                _DEBUG_PRINT("use default netwok-key: %s\n", pCtx->network_key3);
            }
        }		
        ...
        //self-generated WPA2Mixed Tkip+AES
        if (pCtx->is_ap) {
            pCtx->auth_type_flash3 = (WSC_AUTH_WPAPSK | WSC_AUTH_WPA2PSK);
            pCtx->encrypt_type_flash3 = (WSC_ENCRYPT_TKIP | WSC_ENCRYPT_AES);
            pCtx->auth_type3 = WSC_AUTH_WPA2PSK;
            pCtx->encrypt_type3 = WSC_ENCRYPT_AES;

            if (pCtx->manual_key_type) {
                if (pCtx->manual_key_type == 1) {
                    pCtx->auth_type3 = WSC_AUTH_WPAPSK;
                    pCtx->encrypt_type3 = WSC_ENCRYPT_TKIP;					
                    pCtx->auth_type_flash3 = WSC_AUTH_WPAPSK;
                    pCtx->encrypt_type_flash3 = WSC_ENCRYPT_TKIP;					
                } else if (pCtx->manual_key_type == 2) {
                    pCtx->auth_type3 = WSC_AUTH_WPA2PSK;
                    pCtx->encrypt_type3 = WSC_ENCRYPT_AES;	
                    pCtx->auth_type_flash3 = WSC_AUTH_WPA2PSK;
                    pCtx->encrypt_type_flash3 = WSC_ENCRYPT_AES;					
                } else if (pCtx->manual_key_type == 3) {
                    pCtx->auth_type3 = WSC_AUTH_WPA2PSKMIXED;
                    pCtx->encrypt_type3 = WSC_ENCRYPT_TKIPAES;
                    pCtx->auth_type_flash3 = (WSC_AUTH_WPAPSK | WSC_AUTH_WPA2PSK);
                    pCtx->encrypt_type_flash3 = (WSC_ENCRYPT_TKIP | WSC_ENCRYPT_AES);					
                }
            }
        }
        /* wlan0-va0/wlan2 does not support client mode */
        pSta->invoke_security_gen = 1;	
    }
    ...
    byteVal = 1;
    pMsg = add_tlv(pMsg, TAG_NETWORK_KEY_INDEX, 1, &byteVal);
    pMsg = add_tlv(pMsg, TAG_NETWORK_KEY, pCtx->network_key_len3, pCtx->network_key3);
    pMsg = add_tlv(pMsg, TAG_MAC_ADDRESS, ETHER_ADDRLEN, pSta->msg_addr);
    int size1 = (int)(((unsigned long)pMsg) - ((unsigned long)out));
    if (include_cred) {
        shortVal = ntohs(TAG_CREDENTIAL);
	...
	pMsg = add_tlv(pMsg, TAG_SSID, strlen(pCtx->SSID3), pCtx->SSID3);
	TX_DEBUG("SSID3[%s]\n",pCtx->SSID3);	
	AuthType = (pSta->auth_type_flags & pCtx->auth_type_flash3);	
    ...
    if (AuthType) {
        if (AuthType & WSC_AUTH_WPA2PSK)
            ...
            AuthType = pCtx->auth_type3;        
    }
    else {		
        if ((pSta->auth_type_flags & WSC_AUTH_WPA2) && (pCtx->auth_type_flash3 & WSC_AUTH_WPA2PSK))
            AuthType = WSC_AUTH_WPA2PSK;		
        else { // for Intel SDK
            if (pCtx->auth_type_flash3 & WSC_AUTH_WPA2PSK)
                AuthType = WSC_AUTH_WPA2PSK;
            else if (pCtx->auth_type_flash3 & WSC_AUTH_WPAPSK)
                AuthType = WSC_AUTH_WPAPSK;
            else
                AuthType = pCtx->auth_type3;
	...
	EncryType = (pSta->encrypt_type_flags & pCtx->encrypt_type_flash3);	
    if (EncryType) {
        if ( (EncryType & WSC_ENCRYPT_AES) && (AuthType & WSC_AUTH_WPA2PSK))
            EncryType = WSC_ENCRYPT_AES;
        else if((EncryType & WSC_ENCRYPT_TKIP) && (AuthType & WSC_AUTH_WPAPSK))
            ...
            EncryType = pCtx->encrypt_type3;        
    }
    else 
        EncryType = pCtx->encrypt_type3;    

		sec_mix_mode = pCtx->mixedmode3;

    if (sec_mix_mode) {
        if(((pSta->auth_type_flags & WSC_AUTH_WPA2PSK) && (pSta->encrypt_type_flags & WSC_ENCRYPT_AES))
            && (sec_mix_mode & WSC_WPA2_AES)) {
            AuthType=WSC_AUTH_WPA2PSK;
            EncryType=WSC_ENCRYPT_AES;
        } else if(((pSta->auth_type_flags & WSC_AUTH_WPAPSK) && (pSta->encrypt_type_flags & WSC_ENCRYPT_TKIP))
            && (sec_mix_mode & WSC_WPA_TKIP)) {
            AuthType=WSC_AUTH_WPAPSK;
            EncryType=WSC_ENCRYPT_TKIP;
        } else if(((pSta->auth_type_flags & WSC_AUTH_WPAPSK) && (pSta->encrypt_type_flags & WSC_ENCRYPT_AES))
            && (sec_mix_mode & WSC_WPA_AES)) {
            AuthType=WSC_AUTH_WPAPSK;
            EncryType=WSC_ENCRYPT_AES;
        } else if(((pSta->auth_type_flags & WSC_AUTH_WPA2PSK) && (pSta->encrypt_type_flags & WSC_ENCRYPT_TKIP))
            && (sec_mix_mode & WSC_WPA2_TKIP)) {
            AuthType=WSC_AUTH_WPA2PSK;
            EncryType=WSC_ENCRYPT_TKIP;
        } else {
            TX_DEBUG("can't support STA's security capability\n");	
    ...
    /*Fix IOT issue with Ralink dongle */ 
    if (EncryType == WSC_ENCRYPT_WEP && pCtx->wep_transmit_key3)
        byteVal = pCtx->wep_transmit_key3;
    else
        byteVal = 1;
    //---------------------------- 12-18-2008
    ...
    /*2011-0419 , WEP,Hex type Key Issue*/	
    if (EncryType == WSC_ENCRYPT_WEP && pCtx->wep_transmit_key3) {
        pMsg = add_tlv(pMsg, TAG_WEP_TRANSMIT_KEY, 1, &pCtx->wep_transmit_key3);
        convert_char_to_str(pCtx->network_key3,pCtx->network_key_len3,tmp1);
        TX_DEBUG("network key3(string by hex)=%s\n",tmp1);		
    } else {
        TX_DEBUG("network_key3[%s]\n",pCtx->network_key3);					
    }
...
int send_wsc_M8(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	...
	if(pCtx->both_band_credential) {
		...
		if(pCtx->InterFaceComeIn==COME_FROM_WLAN0)
			...
			size = add_credential_attr3(pCtx, pSta, tmpbuf, pCtx->is_ap);
	#endif /* defined(TRIBAND_WPS) */
	}
#else
	size = add_credential_attr(pCtx, pSta, tmpbuf, pCtx->is_ap);
...
// 0930 start
		...
		if (pCtx->InterFaceComeIn == COME_FROM_WLAN2) {
    		p = add_tlv(p, TAG_SSID, strlen(pCtx->SSID3), pCtx->SSID3);		
    		p = add_tlv(p, TAG_MAC_ADDRESS, ETHER_ADDRLEN, pCtx->our_addr3);
    		shortVal = htons(pCtx->auth_type3);
			#ifdef WPS2DOTX			
			if ((pCtx->auth_type3&( WSC_AUTH_OPEN | WSC_AUTH_WPA2 | WSC_AUTH_WPA2PSK | WSC_AUTH_WPA2PSKMIXED ))==0 ) {
    		...
    		shortVal = htons(pCtx->encrypt_type3);
    		...
    		p = add_tlv(p, TAG_NETWORK_KEY, pCtx->network_key_len3, pCtx->network_key3);
    		if (pCtx->encrypt_type3 == WSC_ENCRYPT_WEP) {
    			byteVal = 2;
    			p = add_tlv(p, TAG_NETWORK_KEY_INDEX, 1, &byteVal);		
    			p = add_tlv(p, TAG_NETWORK_KEY, strlen((char *)pCtx->wep_key23), pCtx->wep_key23);
    			byteVal = 3;
    			p = add_tlv(p, TAG_NETWORK_KEY_INDEX, 1, &byteVal);					
    			p = add_tlv(p, TAG_NETWORK_KEY, strlen((char *)pCtx->wep_key33), pCtx->wep_key33);
    			byteVal = 4;
    			p = add_tlv(p, TAG_NETWORK_KEY_INDEX, 1, &byteVal);			
    			p = add_tlv(p, TAG_NETWORK_KEY, strlen((char *)pCtx->wep_key43), pCtx->wep_key43);
    			p = add_tlv(p, TAG_WEP_TRANSMIT_KEY, 1, &pCtx->wep_transmit_key3);
    		}
    	} else
#endif /* defined(TRIBAND_WPS) */
#ifdef FOR_DUAL_BAND
