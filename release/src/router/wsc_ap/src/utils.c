...
int __wlioctl_set_wsc_ie(char *interface, unsigned char *data, int len, int id, int flag)
{
...
#ifdef FOR_DUAL_BAND
	if(pCtx->wps_one_cli_one_daemon){
	...
	if( strstr(interface , WSCD_VAP_INTERFACE) ||strstr(interface , WSCD_WLAN2_INTERFACE) ) {
...
#if defined(TRIBAND_WPS)
	 if (!pCtx->wlan2_wsc_disabled) {
        if (__wlioctl_set_wsc_ie(pCtx->wlan_interface_name3, data, len, id, flag) < 0) {
            WSC_DEBUG("wlioctl_set_wsc_ie fail\n");
            return -1;
        }
    }
...
/* unify root and vxd/vap interface, mib setting in subr_wlan.c */
...
int write_param_to_flash(CTX_Tp pCtx, int is_local)
{
	...
	snprintf(tmpbuf, sizeof(tmpbuf), "INTERFACE=%s\n", pCtx->wlan_interface_name);	
	fputs(tmpbuf, fp);
	
#ifdef FOR_DUAL_BAND
	// if dual band unconfigued, wlan0 & wlan1 may be configured at the same time
	if(pCtx->is_ap && !pCtx->wlan1_wsc_disabled && !pCtx->ProfileDontBothApply){
		UTIL_DEBUG("profile apply to all band\n");		
		snprintf(tmpbuf, sizeof(tmpbuf), "WLAN1_INTERFACE=%s\n", pCtx->wlan_interface_name2);	
		fputs(tmpbuf, fp);
	}
#ifdef TRIBAND_WPS
	// if tri band unconfigued, wlan0 & wlan1 & wlan2 may be configured at the same time
	if(pCtx->is_ap && !pCtx->wlan2_wsc_disabled && !pCtx->ProfileDontBothApply){
		UTIL_DEBUG("profile apply to all band\n");		
		snprintf(tmpbuf, sizeof(tmpbuf), "WLAN2_INTERFACE=%s\n", pCtx->wlan_interface_name3);	
		fputs(tmpbuf, fp);
	}
#endif
#endif
...
//--------------------------------- david+2007-01-26
#if 0 // mark by tesia
		if ((pCtx->assigned_auth_type != WSC_AUTH_OPEN) ||
			(pCtx->assigned_auth_type == WSC_AUTH_OPEN &&
			pCtx->assigned_encrypt_type == WSC_ENCRYPT_NONE)) {
...
#if 0
#ifdef FOR_DUAL_BAND		
	if(pCtx->is_ap && !pCtx->wlan1_wsc_disabled && !pCtx->ProfileDontBothApply && !flash_sync) {		
...
int write_param_to_flash2(CTX_Tp pCtx, int is_local)
{
	...
	// if dual band unconfigued, wlan0 & wlan1 may be configured at the same time
	if(pCtx->is_ap && !pCtx->wlan0_wsc_disabled && !pCtx->ProfileDontBothApply){
		UTIL_DEBUG("profile apply to all band\n");
		snprintf(tmpbuf, sizeof(tmpbuf), "WLAN0_INTERFACE=%s\n", pCtx->wlan_interface_name);	
		fputs(tmpbuf, fp);
	}
#if defined(TRIBAND_WPS)
	// if tri band unconfigued, wlan0 & wlan1 & wlan2 may be configured at the same time
...
sync_flash:
	...
	if(pCtx->is_ap && !pCtx->wlan0_wsc_disabled && !pCtx->ProfileDontBothApply && !flash_sync) {		
		/*is config is assigned by ER ;apply config to both two inteface*/
		UTIL_DEBUG("profile apply to both band\n");
		wlan_idx = 0;
		flash_sync = 1;
...
#if defined(TRIBAND_WPS)
int write_param_to_flash3(CTX_Tp pCtx, int is_local)
{
	char tmpbuf[120];
	FILE *fp;	
	int encrypt, cipher, is_psk=0, len=0;
...
sync_flash:
	if (is_local)
		//MIB_SET(gMIB_WLAN_SSID, pCtx->SSID3);
		sprintf(tmpbuf, "SSID=%s\n", pCtx->SSID3);
	else
		//MIB_SET(gMIB_WLAN_SSID, pCtx->assigned_ssid);
		sprintf(tmpbuf, "SSID=%s\n", pCtx->assigned_ssid);
	...
	if (is_local)
		//MIB_SET(gMIB_WSC_SSID, pCtx->SSID3);
		sprintf(tmpbuf, "WSC_SSID=%s\n", pCtx->SSID3);
	else
		sprintf(tmpbuf, "WSC_SSID=%s\n", pCtx->assigned_ssid);
		//MIB_SET(gMIB_WSC_SSID, pCtx->assigned_ssid);
	...
	encrypt = 0;
	if (is_local) {
		if (pCtx->auth_type_flash3 & WSC_AUTH_WPAPSK)			
			encrypt |= 2;
		...
		fputs(tmpbuf, fp); //write AUTH_TYPE
	}
	...
	if (is_local) {
		if (pCtx->auth_type_flash3 & WSC_AUTH_WPAPSK || pCtx->auth_type_flash3 & WSC_AUTH_WPA2PSK)
			is_psk = 1;			
	} else {
		if (pCtx->assigned_auth_type & WSC_AUTH_WPAPSK || pCtx->assigned_auth_type & WSC_AUTH_WPA2PSK)
			is_psk = 1;
	...
	if (is_psk) {
		if (is_local) {
			...
			fputs(tmpbuf, fp);
			#else
			MIB_SET(gMIB_WLAN_WPA_PSK, pCtx->network_key3);
			#endif
			if (strlen(pCtx->network_key3) == 64)
				//byte = 1;
				fputs("PSK_FORMAT=1\n", fp);
			else
				//byte = 0;
	...
	if (is_local) {
		...
		fputs(tmpbuf, fp);
		#else
		MIB_SET(gMIB_WSC_PSK, pCtx->network_key3);
		#endif
	} else {
		#if 1
		sprintf(tmpbuf, "WSC_PSK=\"%s\"\n", pCtx->assigned_network_key);
	...
	cipher = 0;
	if (is_local) {
		if (pCtx->encrypt_type_flash3 & WSC_ENCRYPT_TKIP)
			cipher |= 1;
		if (pCtx->encrypt_type_flash & WSC_ENCRYPT_AES)
			cipher |= 2;		
	} else {
...
#else
#if 1
	fputs("WPA_CIPHER_SUITE=1\n", fp);  //tkip
	fputs("WPA2_CIPHER_SUITE=2\n", fp); //aes
#else
	byte = 1;		//tkip
	MIB_SET(gMIB_WLAN_WPA_CIPHER_SUITE, &byte);
	byte = 2;		//aes
	...
	cipher = 0;
	if (is_local)
		cipher = pCtx->encrypt_type_flash3;
	else {
		cipher = pCtx->assigned_encrypt_type;
		if (pCtx->assigned_encrypt_type == WSC_ENCRYPT_WEP) {
			unsigned char wepkey[32];
...
void signal_webs(int condition)
{
	...
	if (REINIT_SYS == condition) {
		...
		//system("/bin/reboot"); 
#ifdef WPS_FOR_ASUS
	fprintf(stderr, "echo 1 /tmp/reinit_web\n");
	char tmpbuf[100];
	sprintf(tmpbuf,"echo 1 > %s", REINIT_WEB_FILE);
	system(tmpbuf);
#endif
	}	
#else // CONFIG_RTK_DEV_AP
#ifndef __ECOS
	char tmpbuf[100];
#endif
#ifdef OUTPUT_LOG
					...
					unsigned short method, unsigned char *data)
{
	...
	if (pCtx->three_wps_icons || pCtx->three_wps_icons_param) {
		...
		if ((pCtx->wlan0_wsc_disabled == 0 && pCtx->wlan1_wsc_disabled == 1 && pCtx->wlan2_wsc_disabled == 1) 
			|| (pCtx->wlan0_wsc_disabled == 1 && pCtx->wlan1_wsc_disabled == 1 && pCtx->wlan2_wsc_disabled == 0)) { /*5G*/
			byteVal = 0x02;
			pMsg = add_tlv(pMsg, TAG_RF_BAND, 1, (void *)&byteVal);
		} else if(pCtx->wlan0_wsc_disabled == 1 && pCtx->wlan1_wsc_disabled == 0 && pCtx->wlan2_wsc_disabled == 1) { /*2.4G*/
			byteVal = 0x01;
			pMsg = add_tlv(pMsg, TAG_RF_BAND, 1, (void *)&byteVal);
		}
	} else
...
void report_WPS_STATUS(int status)
{
...
#elif defined(WPS_FOR_ASUS)
	FILE *fp = NULL;
	fp = fopen(WSCD_CONFIG_STATUS, "w");
	if(fp) {
		fprintf(fp, "%d", status);
		fclose(fp);
	}
	...
	if (!pCtx->wlan0_wsc_disabled && !pCtx->wlan1_wsc_disabled) {
		sprintf(tmp, "echo %d > %s-%s-%s", status, WSCD_CONFIG_STATUS,pCtx->wlan_interface_name,pCtx->wlan_interface_name2);
	} else if (!pCtx->wlan0_wsc_disabled) {
		sprintf(tmp, "echo %d > %s-%s", status, WSCD_CONFIG_STATUS,pCtx->wlan_interface_name);	
	} else if (!pCtx->wlan1_wsc_disabled) {
		sprintf(tmp, "echo %d > %s-%s", status, WSCD_CONFIG_STATUS,pCtx->wlan_interface_name2); 	
	} else if(!pCtx->wlan2_wsc_disabled) {
		sprintf(tmp, "echo %d > %s-%s", status, WSCD_CONFIG_STATUS,pCtx->wlan_interface_name3); 
	} else
#endif /* defined(TRIBAND_WPS) */
